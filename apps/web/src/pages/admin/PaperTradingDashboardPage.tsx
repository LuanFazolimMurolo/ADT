...
function runnerClass(status: PaperRunnerCycleStatus | PaperRunnerSessionStatus): string {
  if (status === 'FAILED') return 'paper-status paper-status--danger'
  if (status === 'PARTIALLY_FAILED') return 'paper-status paper-status--warning'
  if (status === 'UPDATED' || status === 'COMPLETED') return 'paper-status paper-status--success'
  return 'paper-status'
}

function sessionState(session: PaperDashboardSession): { label: string; className: string } {
  if (!session.state_available) return { label: 'Pendente', className: 'paper-status' }
  if (session.risk_halt) {
    return { label: 'Risco interrompido', className: 'paper-status paper-status--danger' }
  }
  if (session.runner?.status === 'FAILED') {
    return { label: 'Runner falhou', className: 'paper-status paper-status--danger' }
  }
  return { label: 'Inicializada', className: 'paper-status paper-status--success' }
}
...
function SessionCard({
  ...
  return (
    <article className={selected ? 'paper-session paper-session--selected' : 'paper-session'}>
      <header className="paper-session__header">
        <div>
          <p className="eyebrow">{session.timeframe} · {session.strategy_name}@{session.strategy_version}</p>
      ...
      <div className="paper-session__badges">
        ...
        {session.runner && !session.runner.matches_current_state && session.runner.status !== 'FAILED' && (
          <span className="paper-status paper-status--warning">Estado defasado</span>
        )}
      </div>
      ...
      {metrics ? (
        <div className="paper-session__metrics">
          ...
          <div>
            <span>PnL total</span>
            <strong className={isNegativeDecimal(metrics.total_pnl) ? 'value-negative' : 'value-positive'}>
              {formatMoney(metrics.total_pnl, session.quote_asset)}
            </strong>
          </div>
          <div>
            <span>Retorno</span>
            <strong className={isNegativeDecimal(metrics.return_pct) ? 'value-negative' : 'value-positive'}>
        ...
        <div className="paper-session__pending">
          ...
          <span>Capital configurado: {formatMoney(session.initial_capital, session.quote_asset)}</span>
        </div>
      )}
      ...
      <dl className="paper-session__details">
        <div>
          <dt>Posição</dt>
          <dd>{position?.is_open ? `${formatExactDecimal(position.base_quantity, 0)} ${session.base_asset}` : 'Fechada'}</dd>
        </div>
        <div>
...
function ComparisonPanel({ sessions }: { sessions: PaperDashboardSession[] }) {
  ...
  const rows = [
    ['Equity', (session: PaperDashboardSession) => session.metrics ? formatMoney(session.metrics.equity, session.quote_asset) : '—'],
    ['PnL total', (session: PaperDashboardSession) => session.metrics ? formatMoney(session.metrics.total_pnl, session.quote_asset) : '—'],
    ['Retorno', (session: PaperDashboardSession) => session.metrics ? formatPercentage(session.metrics.return_pct) : '—'],
    ['Drawdown', (session: PaperDashboardSession) => session.metrics ? formatPercentage(session.metrics.drawdown_pct) : '—'],
    ['Posição', (session: PaperDashboardSession) => session.position ? (session.position.is_open ? 'Aberta' : 'Fechada') : '—'],
    ['Ordens abertas', (session: PaperDashboardSession) => String(session.open_orders_count)],
    ['Regime', (session: PaperDashboardSession) => session.latest_market_regime ? regimeLabels[session.latest_market_regime.regime] : '—'],
  ] as const

  return (
    <section className="paper-comparison" aria-labelledby="paper-comparison-title">
      <div className="section-heading">
        <div>
          <p className="eyebrow">Comparação local</p>
          <h2 id="paper-comparison-title">Sessões selecionadas</h2>
        </div>
        <span>{sessions.length === 1 ? 'Selecione mais uma sessão' : 'Duas sessões selecionadas'}</span>
      </div>
      <div className="table-wrap">
        <table>
          <thead>
            <tr>
              <th>Métrica</th>
              {sessions.map((session) => <th key={session.session_id}>{sessionLabel(session)}</th>)}
            </tr>
          </thead>
          <tbody>
            {rows.map(([label, value]) => (
              <tr key={label}>
                ...
                {sessions.map((session) => <td key={session.session_id}>{value(session)}</td>)}
              </tr>
            ))}
          </tbody>
        </table>
...
export function PaperTradingDashboardPage() {
  ...
  const [dashboard, setDashboard] = useState<PaperDashboardResponse | null>(null)
  ...
  const load = useCallback(async (initial: boolean) => {
    ...
    try {
      const result = await apiClient.getPaperTradingDashboard(page, PAGE_SIZE)
      if (sequence !== requestSequence.current) return
      setDashboard(result)
      setSelectedIds((current) => current.filter((id) => result.items.some((item) => item.session_id === id)))
      setUpdatedAt(new Date())
      setError(null)
    } catch (nextError) {
      if (sequence !== requestSequence.current) return
      setError(getErrorMessage(nextError, 'Não foi possível carregar o dashboard de paper trading.'))
    } finally {
    ...
    <div>
      <div className="page-heading paper-dashboard-heading">
        <div>
          ...
          <h1>Paper trading</h1>
          <p>Performance, posição e execução das sessões persistidas pelo runner.</p>
        </div>
      ...
      {loading ? <LoadingState message="Carregando performance do paper trading…" /> : error && !dashboard ? (
        ...
        <>
          ...
          <section className="paper-runner-strip" aria-label="Estado do runner">
            <div>
              <span className={dashboard.runner ? runnerClass(dashboard.runner.status) : 'paper-status'}>
                {dashboard.runner ? runnerCycleLabels[dashboard.runner.status] : 'Aguardando'}
              </span>
              <div>
                <small>RUNNER</small>
                <strong>{dashboard.runner ? `Ciclo ${dashboard.runner.cycle_index}` : 'Nenhum ciclo persistido'}</strong>
              </div>
            </div>
            <div>
              <small>ÚLTIMA EXECUÇÃO</small>
              <strong>{formatDate(dashboard.runner?.finished_at ?? null)}</strong>
            </div>
            <div>
              <small>PRÓXIMO CICLO</small>
              <strong>{formatDate(dashboard.runner?.next_cycle_at ?? null)}</strong>
            </div>
            <div>
          ...
          <section className="section-heading">
            ...
            <span>Valores nominais; as sessões podem usar ativos de cotação diferentes.</span>
          </section>
          ...
          <section className="paper-totals-grid">
            <article className="metric-card">
              ...
              <small>{dashboard.totals.initialized_count} inicializadas · {dashboard.totals.pending_count} pendentes</small>
            </article>
            ...
            <article className={isNegativeDecimal(dashboard.totals.total_pnl) ? 'metric-card metric-card--negative' : 'metric-card'}>
              <span className="metric-label">PNL NOMINAL</span>
              <strong>{formatExactDecimal(dashboard.totals.total_pnl)}</strong>
              <small>Retorno {formatPercentage(dashboard.totals.return_pct)}</small>
            </article>
            <article className="metric-card">
              <span className="metric-label">DRAWDOWN MÁXIMO</span>
              <strong>{formatPercentage(dashboard.totals.maximum_drawdown_pct)}</strong>
            ...
            <article className={dashboard.totals.runner_failed_count || dashboard.totals.risk_halted_count ? 'metric-card metric-card--negative' : 'metric-card'}>
              <span className="metric-label">ALERTAS</span>
              <strong>{dashboard.totals.runner_failed_count + dashboard.totals.risk_halted_count}</strong>
              <small>{dashboard.totals.runner_failed_count} falhas · {dashboard.totals.risk_halted_count} risk halts</small>
            </article>
          </section>
